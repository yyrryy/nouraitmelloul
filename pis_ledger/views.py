...
from pis_product.models import Facture
...
class CustomerLedgerView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        clientswithdebts=Customer.objects.filter(hasdebt=True)
        debt=Facture.objects.filter(ispaid=False, client__in=clientswithdebts).aggregate(Sum('total'))['total__sum'] or 0
        customer_ledger = []
